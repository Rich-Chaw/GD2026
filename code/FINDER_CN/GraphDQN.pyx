...
#################################################New code for graphDQN#####################################
    def BuildNet(self):
     ...
     #   embed_s_a = tf.concat([action_embed,y_potential],1)
        ...
        loss_recons = 2 * tf.compat.v1.trace(tf.matmul(tf.transpose(cur_message_layer), tf.sparse_tensor_dense_matmul(tf.cast(self.laplacian_param,tf.float32), cur_message_layer)))
