...
std::vector<int> Utils::reverseGreedy(std::shared_ptr<Graph> graph, std::vector<int> solution, const std::vector<int> allVex, double retainRatio)
{
    ...
    {
        graphutil->recoverAddNode(backupCompletedAdjListGraph, currentAllVex, current_adj_list, eachV, disjoint_Set);
    }
    ...
    for (std::vector<int>::reverse_iterator it = solution.rbegin(); it != solution.rend(); ++it)
    {
        ...
        Disjoint_Set disjoint_Set_before = disjoint_Set;

        std::vector<std::vector<int> > current_adj_list_before = current_adj_list;

        std::vector<bool> currentAllVex_before = currentAllVex;

        graphutil->recoverAddNode(backupCompletedAdjListGraph, currentAllVex, current_adj_list, Node, disjoint_Set);

        double left_mcc_ratio = (double)disjoint_Set.maxRankCount / (double)graph->num_nodes;

        if (left_mcc_ratio > retainRatio)
        {
            disjoint_Set = disjoint_Set_before;

            current_adj_list = current_adj_list_before;

            currentAllVex = currentAllVex_before;
        }
        else
        {
    ...
    {
        if(deleteSet.count(solution[i])==0)
        {
